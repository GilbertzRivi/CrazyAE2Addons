...
import net.oktawia.crazyae2addonslite.menus.AmpereMeterMenu;
...
public class AmpereMeterScreen<C extends AmpereMeterMenu> extends AEBaseScreen<C> {
    ...
    public AmpereMeterScreen(C menu, Inventory playerInventory, Component title, ScreenStyle style) {
        ...
        if (IsModLoaded.isGTCEuLoaded()){
            minFe.setTooltipMessage(List.of(Component.literal("Min power in FE/t or GT amperes (emits redstone strength 1)")));
        } else {
            minFe.setTooltipMessage(List.of(Component.literal("Min FE/t (emits redstone strength 1)")));
        }
        ...
        if (IsModLoaded.isGTCEuLoaded()) {
            maxFe.setTooltipMessage(List.of(Component.literal("Max power in FE/t or GT amperes (emits redstone strength 15)")));
        } else {
            maxFe.setTooltipMessage(List.of(Component.literal("Max FE/t (emits redstone strength 15)")));
        }
