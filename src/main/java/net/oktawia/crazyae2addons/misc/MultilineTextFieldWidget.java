...
public class MultilineTextFieldWidget extends AbstractWidget {
    ...
    public boolean mouseScrolled(double mx, double my, double scrollX, double scrollY) {
        ...
        setScrollAmount(scrollAmount - scrollY * font.lineHeight);
        return true;
    }
    ...
    protected void renderWidget(GuiGraphics g, int mX, int mY, float partial) {
            ...
            if (textField.hasSelection()) {
                ...
                if (!(selectionEnd <= lineStartChar || selectionBegin >= lineEndChar)) {
                    ...
                    if (selStartInLine < selEndInLine) {
                        ...
                        String selText = str.substring(selStartInLine, selEndInLine);
                        int selX = xOff + font.width(preSel);
                        int selW = font.width(selText);
        ...
        // Scrollbar
        int totalPx = textField.lineCount() * font.lineHeight;
        int viewPx  = height - 4;
        if (totalPx > viewPx) {
            int barX    = getX() + width - 4;
            int barTop  = getY() + 2;
            int barH    = height - 4;
            int thumbH  = Math.max(8, barH * viewPx / totalPx);
            int maxScrl = totalPx - viewPx;
            int thumbY  = barTop + (int) ((barH - thumbH) * Math.min(scrollAmount, maxScrl) / maxScrl);
            g.fill(barX, barTop, barX + 2, barTop + barH, 0xFF404040);
            g.fill(barX, thumbY, barX + 2, thumbY + thumbH, 0xFFAAAAAA);
        }
    }
