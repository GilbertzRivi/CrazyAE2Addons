...
public final class PenroseExplosionTask {

    private static final BlockState AIR = Blocks.AIR.defaultBlockState();

    private static final long WORK_NANOS_PER_TICK = 100_000_000L;
    ...
    public PenroseExplosionTask(ServerLevel level, BlockPos center, int radiusBlocks) {
        this(level, UUID.randomUUID(), center, radiusBlocks, null);
    }

    public PenroseExplosionTask(ServerLevel level, UUID id, BlockPos center, int radiusBlocks, long[] alreadyProcessed) {
        this.level = level;
        this.id = id;
        this.center = center.immutable();
        this.radiusBlocks = radiusBlocks;
        this.radiusSq = (long) radiusBlocks * (long) radiusBlocks;
        ...
        for (ChunkPos chunkPos : chunkOrder) {
            targetChunks.add(chunkPos.toLong());
        }

        this.sectionOrder = buildSectionOrder();

        if (alreadyProcessed != null) {
            for (long value : alreadyProcessed) {
    ...
    public void onChunkLoaded(ChunkPos chunkPos) {
        ...
        if (curChunkPos != null && curChunkPos.toLong() == key) {
            return;
        }

        queuedChunks.add(key);
        workQueue.addLast(chunkPos);
    }
    ...
    private boolean step() {
        ...
        if (curChunk == null) {
            ...
            while (tries-- > 0 && !workQueue.isEmpty()) {
                ChunkPos chunkPos = workQueue.pollFirst();
                long key = chunkPos.toLong();
        ...
        while (curSectionCursor < sectionOrder.length) {
            int sectionIndex = sectionOrder[curSectionCursor++];
            ...
            LevelChunkSection section = curChunk.getSection(sectionIndex);
            if (section == null) {
                continue;
            }
            ...
            curSection = section;
            curSectionBottomY = sectionBottomY;
            lx = 0;
            ly = 0;
            lz = 0;
            return true;
        ...
        if (distSq <= radiusSq) {
            ...
            if (!old.isAir() && !old.is(Blocks.BEDROCK)) {
                ...
                if (old.hasBlockEntity()) {
                    curChunk.removeBlockEntity(mutablePos);
                }
    ...
    private boolean isChunkStillLoaded(ChunkPos chunkPos) {
        if (chunkPos == null) {
            return false;
        }
    ...
    private void seedCurrentlyLoadedChunks() {
        for (ChunkPos chunkPos : chunkOrder) {
            ...
            LevelChunk chunk = level.getChunkSource().getChunkNow(chunkPos.x, chunkPos.z);
            if (chunk != null && queuedChunks.add(key)) {
                workQueue.addLast(chunkPos);
            }
        }
    }
    ...
    private int[] buildSectionOrder() {
        ...
        for (int i = 0; i < count; i++) {
            tmp[i] = minSectionIndex + i;
        }

        Arrays.sort(tmp, Comparator.comparingInt(sectionIndex -> Math.abs(sectionIndex - centerSection)));
    ...
    private static List<ChunkPos> buildChunkOrder(BlockPos center, int radiusBlocks) {
        ...
        for (int cx = centerChunkX - chunkRadius; cx <= centerChunkX + chunkRadius; cx++) {
            for (int cz = centerChunkZ - chunkRadius; cz <= centerChunkZ + chunkRadius; cz++) {
                ...
                if (center.getX() < chunkMinX) {
                    dx = chunkMinX - center.getX();
                } else if (center.getX() > chunkMaxX) {
                    dx = center.getX() - chunkMaxX;
                }

                long dz = 0;
                if (center.getZ() < chunkMinZ) {
                    dz = chunkMinZ - center.getZ();
                } else if (center.getZ() > chunkMaxZ) {
                    dz = center.getZ() - chunkMaxZ;
                }
        ...
        out.sort(Comparator.comparingLong(chunkPos -> {
            long ccx = ((long) chunkPos.x << 4) + 8L;
            long ccz = ((long) chunkPos.z << 4) + 8L;
    ...
    private static long minDistanceSqToBox(
