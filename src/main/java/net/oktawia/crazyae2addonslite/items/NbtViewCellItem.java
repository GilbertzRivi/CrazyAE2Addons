...
import net.oktawia.crazyae2addonslite.logic.ViewCellHost;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;

public class NbtViewCellItem extends ViewCellItem implements IMenuItem {
    public NbtViewCellItem(Properties props) { super(props.stacksTo(1)); }

    @Override
    public @NotNull InteractionResultHolder<ItemStack> use(
            @NotNull Level level, @NotNull Player p, @NotNull InteractionHand hand) {
        if (!level.isClientSide()) {
            MenuOpener.open(CrazyMenuRegistrar.NBT_VIEW_CELL_MENU.get(), p, MenuLocators.forHand(p, hand));
        }
