...
public class PortableSpatialStorageScreen<C extends PortableSpatialStorageMenu> extends AEBaseScreen<C> {
    ...
    private boolean previewStructureFromSharedCache = false;
    ...
    private PortableSpatialStorageSceneWidget scene;

    private final AETextField offsetDisplayX;
    private final AETextField offsetDisplayY;
    private final AETextField offsetDisplayZ;
    ...
    private int previewReloadDelay = -1;
    ...
    public PortableSpatialStorageScreen(C menu, Inventory playerInventory, Component title, ScreenStyle style) {
        ...
        IconButton offsetLeftButton = new IconButton(Icon.ARROW_LEFT, button -> {
            getMenu().offsetLeft();
        });
        offsetLeftButton.setTooltip(Tooltip.create(
                Component.translatable(LangDefs.OFFSET_LEFT_TOOLTIP.getTranslationKey())
        ));
        this.widgets.add("offsetLeft", offsetLeftButton);

        IconButton offsetRightButton = new IconButton(Icon.ARROW_RIGHT, button -> {
            getMenu().offsetRight();
        });
        offsetRightButton.setTooltip(Tooltip.create(
                Component.translatable(LangDefs.OFFSET_RIGHT_TOOLTIP.getTranslationKey())
        ));
        this.widgets.add("offsetRight", offsetRightButton);

        IconButton offsetUpButton = new IconButton(Icon.ARROW_UP, button -> {
            getMenu().offsetUp();
        });
        ...
        IconButton offsetDownButton = new IconButton(Icon.ARROW_DOWN, button -> {
            getMenu().offsetDown();
        });
        ...
        IconButton offsetFrontButton = new IconButton(Icon.SCHEDULING_DEFAULT, button -> {
            getMenu().offsetFront();
        });
        offsetFrontButton.setTooltip(Tooltip.create(
                Component.translatable(LangDefs.OFFSET_FRONT_TOOLTIP.getTranslationKey())
        ));
        this.widgets.add("offsetFront", offsetFrontButton);

        IconButton offsetBackButton = new IconButton(Icon.SCHEDULING_DEFAULT, button -> {
            getMenu().offsetBack();
        });
        offsetBackButton.setTooltip(Tooltip.create(
                Component.translatable(LangDefs.OFFSET_BACK_TOOLTIP.getTranslationKey())
        ));
        ...
        this.offsetDisplayX.setTooltipMessage(List.of(
                Component.translatable(LangDefs.OFFSET_X_TOOLTIP.getTranslationKey())
        ));
        ...
        this.offsetDisplayY.setTooltipMessage(List.of(
                Component.translatable(LangDefs.OFFSET_Y_TOOLTIP.getTranslationKey())
        ));
        ...
        this.offsetDisplayZ.setTooltipMessage(List.of(
                Component.translatable(LangDefs.OFFSET_Z_TOOLTIP.getTranslationKey())
        ));
    ...
    private void syncOffsetDisplays() {
    ...
    private BlockPos readCurrentOffset() {
        ...
        if (stack.isEmpty()) {
            stack = PortableSpatialStorage.findHeld(player);
        }
    ...
    public void reloadPreviewNow() {
        ...
        var stack = PortableSpatialStorage.findActive(player);
        if (stack.isEmpty()) {
            stack = PortableSpatialStorage.findHeld(player);
        }
        ...
        this.scene.setPreview(
                newStructure,
                CutPasteStackState.getPreviewSideMap(stack),
                this.renderedCore,
                originMarkerPos,
                floorAnchorPos
        );
