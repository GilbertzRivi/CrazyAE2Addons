...
import net.oktawia.crazyae2addonslite.interfaces.IStatefulTokenizer;
...
public class MultilineTextFieldWidget extends AbstractWidget {
    ...
    public boolean mouseScrolled(double mx, double my, double delta) {
        ...
        final double LINES_PER_TICK = 1;
        double sens = Minecraft.getInstance().options.mouseWheelSensitivity().get();
        if (sens <= 0) sens = 1.0;
        double ticks = delta / sens;
        setScrollAmount(scrollAmount - ticks * font.lineHeight * LINES_PER_TICK);
        return true;
    }
