...
public final class DisplayTokenResolver {
    ...
    private static final long MAX_RATE_WINDOW_TICKS = 20L * 60L * 30L; // 30 min
    ...
    public static void recomputeVariablesAndNotify(DisplayPart part) {
        ...
        if (!deltaTokens.isEmpty()) {
            ...
            for (String id : deltaIds) {
                if (!amounts.containsKey(id)) continue;
                if (part.gridWarmupRemaining > 0) continue;
        ...
        for (String token : stockTokens) {
            ...
            if (!amounts.containsKey(itemId)) continue;
    ...
    private static Map<String, Long> getAmountsInME(DisplayPart part, Set<String> ids) {
        ...
        try {
            ...
            for (var gs : avail) byKey.merge(gs.getKey(), gs.getLongValue(), Long::sum);
    ...
    public static AEKey resolveKey(String rawId) {
        ...
        var rl = ResourceLocation.parse(rawId);
        var item = BuiltInRegistries.ITEM.getOptional(rl).orElse(null);
        if (item != null && item != Items.AIR) return AEItemKey.of(item);
    ...
    public static String formatSignedNumber(double v) {
        ...
        long r = Math.round(v);
        if (Math.abs(v - r) < 1e-9) return (r > 0 ? "+" : "") + r;
