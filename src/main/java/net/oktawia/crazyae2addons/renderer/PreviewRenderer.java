...
public class PreviewRenderer {
    ...
    public static void render(PreviewInfo previewInfo, RenderLevelStageEvent event) {
        ...
        if (pointed != null) {
            PreviewTooltipRenderer.text = pointed.state().getBlock().getName().getString();
        }
        if (!stillFocused) {
        ...
        for (Map.Entry<Integer, Float> entry : previewInfo.alpha.entrySet()) {
            ...
            if (previewInfo.focusY == null) {
                if (value > previewAlpha) {
                    previewInfo.alpha.put(key, max(previewAlpha, previewInfo.alpha.get(key) - deltaTick * alphaStep));
                }
                if (value < previewAlpha) {
                    previewInfo.alpha.put(key, min(previewAlpha, previewInfo.alpha.get(key) + deltaTick * alphaStep));
                }
            } else {
                if (key == previewInfo.focusY) {
                    if (value < layeredAlpha) {
                        previewInfo.alpha.put(key, min(layeredAlpha, previewInfo.alpha.get(key) + deltaTick * alphaStep));
                    }
                } else {
                    if (value > 0) {
                        previewInfo.alpha.put(key, max(0, previewInfo.alpha.get(key) - deltaTick * alphaStep));
                    }
                }
            }
        }
        ...
        for (PreviewInfo.BlockInfo info : visibleBlocks) {
            ...
            float alpha = previewInfo.alpha.computeIfAbsent(pos.getY(), k -> previewInfo.focusY == null ? previewAlpha : layeredAlpha);
            if (alpha > 0) {
